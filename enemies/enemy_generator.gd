...
func _ready() -> void:
	red_enemy_spawn_timer.timeout.connect(handle_spawn.bind(RedEnemeyScene, red_enemy_spawn_timer, 3));
	green_enemy_spawn_timer.timeout.connect(handle_spawn.bind(GreenEnemeyScene, green_enemy_spawn_timer, 10.0));
	blue_enemy_spawn_timer.timeout.connect(handle_spawn.bind(BlueEnemyScene, blue_enemy_spawn_timer, 15.0));
...
func handle_spawn(enemy_scene: PackedScene, timer: Timer, time_offset: float = 1.0) -> void:
	...
	var spawn_rate = time_offset / (0.4 + (game_stats.score * 0.01))
